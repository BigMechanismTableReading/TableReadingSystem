...
public class Extraction {
	...
	private Pair<HashMap<Integer,String>, HashMap<Integer,String>> getAllParticipantB(HashMap<ColumnContents, List<TableBuf.Column>> contents){
		...
		for(ColumnContents c : contents.keySet()){
			...
			if(c instanceof GOid && go_col == null){
				go_col = contents.get(c).get(0);
				go = (GOid) c;
			}
		}
		...
		int row = 0;
		if(col != null || go_col != null){
			if(TableReader.go_only){
				while(row < go_col.getDataCount()){	
					Pair<String, String> ground = go.get_data(go_col, row);
					if(ground != null){
						if(ground.getB() != null) {
